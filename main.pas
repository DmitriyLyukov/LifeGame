...
function NeighborsInTor(var A:Field; i,j:integer):integer;
 ...
 begin
  ...
  for l:=1 to 9 do begin
   x:=i-1+(l-1) div 3;
   y:=j-1+(l+2) mod 3;
   if x=-1 then x:=m-1 else if x=m then x:=0;
   if y=-1 then y:=n-1 else if y=n then y:=0;
   if A[x,y]='#' then sum:=sum+1;
                   end;
  if A[i,j]='#' then sum:=sum-1;
  NeighborsInTor:=sum;
 end;
...
function neighbors(var A:Field; i,j:integer; InTor:boolean):integer;
 ...
 begin
  if  InTor
    then neighbors:=NeighborsInTor(A,i,j)
    else begin
  sum:=0;
  for l:=1 to 9 do begin
   x:=i-1+(l-1) div 3;
...
procedure step(var A:Field; InTor:boolean);
 ...
 begin
  ...
  for i:=0 to m-1 do
   for j:=0 to n-1 do
    if (A[i,j]='#') and not(neighbors(B,i,j,InTor) in [2,3]) then A[i,j]:='_'
     else if (A[i,j]<>'#') and (neighbors(B,i,j,InTor)=3) then A[i,j]:='#';
 end;
...
procedure TForm1.PathChange(Sender: TObject);
begin
...
procedure TForm1.dgSelectCell(Sender: TObject; aCol, aRow: Integer;
  var CanSelect: Boolean);
begin
  if A[aRow,aCol]='#' then A[aRow,aCol] := '_' else A[aRow,aCol] := '#';
end;
...
begin
  if SelectFile.Execute then Path.Text:=SelectFile.FileName;
end;
